...
from papaya.util import getViewURL
...
def endpoint(request):
    ...
    if openid_request.mode in ["checkid_immediate", "checkid_setup"]:
        if not request.user.is_authenticated():
            if request.POST:
                full_path = request.path + "?" + request.POST.urlencode()
            else:
                full_path = request.get_full_path()
            tup = settings.LOGIN_URL, REDIRECT_FIELD_NAME, urlquote(full_path)
...
def handleCheckIDRequest(request, openid_request):
    ...
    if not openid_request.idSelect():
        ...
        serverUrl = getViewURL(request, server)
        userId = None
        if openid_request.identity.startswith(serverUrl):
            userId = openid_request.identity.rstrip('/').replace(serverUrl, '')

        # Confirm that this server can actually vouch for that
        # identifier
        if not userId or userId != request.user.username:
